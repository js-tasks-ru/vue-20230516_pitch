<template>
  <form
    class="meetup-form"
    @submit="$emit('submit', klona(localMeetup))"
  >
    <div class="meetup-form__content">
      ...
      <meetup-agenda-item-form
        ...
        :key="agendaItem.id"
        v-model:agenda-item="localMeetup.agenda[idx]"
...
export default {
  ...
  watch: {
    localMeetup: {
      ...
      immediate: true,
      handler() {
        this.$emit('update:meetup', klona(this.meetup));
      },
    },
  },
  ...
  methods: {
    ...
    addingAgendaItem() {
      ...
      if (this.localMeetup.agenda.length) {
        agenda.startsAt = this.localMeetup.agenda.at(-1).endsAt
        agenda.endsAt = this.localMeetup.agenda.at(-1).endsAt
      }
